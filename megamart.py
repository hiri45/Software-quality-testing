...
def is_not_allowed_to_purchase_item(item: Item, customer: Customer, purchase_date_string: str) -> bool:
  ...
  if any(category in restricted_categories for category in item_categories): # if the customer buy the restricted category 
    if customer is None: # No customer link
      return True
    if customer.date_of_birth is None: # No birthdate
      return True
    if purchase_date_string is None: # No purchase date
      return True
    ...
    birth_date = ""
    try:
      purchase_date = datetime.strptime(purchase_date_string,format_date)
    except ValueError:
    ...
    if round(purchase_date.year - birth_date.year) < 18: # Member account not provided for restricted item
...
def calculate_fulfilment_surcharge(fulfilment_type: FulfilmentType, customer: Customer) -> float:
  ...
  """
  # Exceptions
  if fulfilment_type == None: 
    raise Exception("The fullfilment_type are not provided")
  if fulfilment_type != FulfilmentType.DELIVERY: # Choose anything other than delivery then return 0
    return 0.00
  if customer == None:
    raise FulfilmentException("The customer delivery are not provided")
  if customer.delivery_distance_km == None:
    raise FulfilmentException("The delivery distance are not provided")

  if round(customer.delivery_distance_km*0.5,2) <= 5.0: # if the amount less equal than the 5.0
